...
class Mouse {
  constructor(element) {_defineProperty(this, "reset",
    ...
    });this.element = element || window;this.drag = false;this.x = cwidth / 2;this.y = cheight / 2;this.pointer = this.pointer.bind(this);this.getCoordinates = this.getCoordinates.bind(this);this.events = ["mouseenter", "mousemove"];this.events.forEach(eventName => {this.element.addEventListener(eventName, this.getCoordinates);});this.element.addEventListener("mousedown", () => {this.drag = true;});this.element.addEventListener("mouseup", () => {this.drag = false;});}
...
class Render {
  constructor() {_defineProperty(this, "createShader",
    ...
    () => {
      ...
      this.umouse = [
      -(this.canvas.width - mouse.x),
      -(this.canvas.height - mouse.y),
      0];
    ...
    let _width = this.width = cwidth;let _height = this.height = cheight;const canvas = this.canvas = document.createElement("canvas");const container = document.getElementById("container");canvas.id = "GLShaders";canvas.width = _width;canvas.height = _height;this.mouse = new Mouse(canvas);container.appendChild(canvas);const gl = this.gl = canvas.getContext("webgl2");if (!gl) {console.warn("WebGL 2 is not available.");return;} // WebGl and WebGl2 Extension //
    ...
    window.addEventListener("resize", () => {this.canvas.width = cwidth;this.canvas.height = cheight;this.gl.viewport(0, 0, this.canvas.width, this.canvas.height);this.resolution = new Float32Array([this.canvas.width, this.canvas.height]);this.gl.uniform2fv(this.gl.getUniformLocation(this.program, "resolution"), this.resolution);this.clearCanvas();}, false);this.init();} // Shader Bootstrap code //
